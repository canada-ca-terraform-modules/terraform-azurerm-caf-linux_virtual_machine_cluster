...
resource "azurerm_resource_group" "live_test" {
  ...
  name     = "${var.env}-caf-linux-vm-cluster-live-test-${var.pr_number}-rg"
...
resource "azurerm_virtual_network" "live_test" {
  name                = "${var.env}-caf-linux-vm-cluster-live-test-${var.pr_number}-vnet"
...
resource "azurerm_subnet" "live_test" {
  name                 = "${var.env}-caf-linux-vm-cluster-live-test-${var.pr_number}-snet"
  resource_group_name  = azurerm_resource_group.live_test.name
...
locals {
  # The child linux_virtual_machine module computes a unique suffix via
